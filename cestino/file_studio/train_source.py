...
def train_one(args):
    ...
    print(f"[{proc_name}] Inizio: {env_id_v1} (seed={seed})")
    ...
    try:
        env = make_vec_env(
            ...
            n_envs=N_ENVS, 
            seed=seed,
            monitor_dir=log_dir  # <--- I file .monitor.csv andranno qui
        )
        ...
        model.learn(
            ...
            tb_log_name=f"PPO_{env_id_v1}",
            progress_bar=False,
        )

        save_path = os.path.join(save_dir, f"PPO_{env_id_v1}_s{seed}")
        model.save(save_path)
        env.close()

        print(f"[{proc_name}] ✓ Completato: {env_id_v1} → {save_path}")
        return env_id, True
...
if __name__ == "__main__":
    ...
    for env_id in ENV_IDS:
        for s in range(N_SEEDS):
            # Usiamo seed diversi per ogni ripetizione
