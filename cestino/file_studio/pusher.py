    ...
    * v0: Initial versions release.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Ottiene la cartella dove si trova questo file (pusher.py)
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # Unisce la cartella al nome del file XML per avere il percorso completo
        full_xml_path = os.path.join(current_dir, xml_file)
        # -----------------------------

        utils.EzPickle.__init__(
            self,
            xml_file, # Qui puoi lasciare il nome semplice per Pickle
            frame_skip,
            default_camera_config,
            reward_near_weight,
            reward_dist_weight,
        ...
        MujocoEnv.__init__(
            self,
            full_xml_path,  # <--- CAMBIA xml_file con full_xml_path qui!
            frame_skip,
    ...
    def reset_model(self):
        ...
        while True:
            self.cylinder_pos = np.concatenate(
                [
                    self.np_random.uniform(low=-0.6, high=-0.3, size=1),   # x più conservativo
                    self.np_random.uniform(low=-0.3, high=0.3, size=1) # y più conservativo
                ]
            )
